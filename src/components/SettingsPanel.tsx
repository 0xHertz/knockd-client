...
export default function SettingsPanel({ onClose }: Props) {
  ...
  useEffect(() => {
    (async () => {
      const [dc, dd, cli] = await Promise.all([
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-sm mx-4 rounded-2xl bg-slate-900 border border-slate-700/50 shadow-2xl">
            ...
            <div className="space-y-1 max-h-32 overflow-y-auto">
              {clients.map((c) => (
                ...
                >
                  ...
                  <span
                    className={c.installed ? "text-emerald-400" : "text-slate-600"}
                  >
