...
export default function SettingsPanel({ onClose }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setClients(cli);
      if (cp) {
        try { setCustomEntries(JSON.parse(cp)); } catch { /* ignore */ }
      }
    })();
  }, []);
  ...
  const browseFile = async (idx: number) => {
    const selected = await open({
      multiple: false,
      filters: [{ name: "Executable", extensions: ["exe", "bat", "cmd", "com"] }],
    });
    if (selected) {
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-md mx-4 rounded-2xl bg-slate-900 border border-slate-700/50 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Default SSH Client</label>
            <select value={defaultClient} onChange={(e) => setDefaultClient(e.target.value)}
              className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-600/50 text-sm focus:outline-none focus:border-emerald-500/50">
              <option value="auto">Auto-detect</option>
              {clients.filter((c) => c.installed).map((c) => (
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Default Knock Delay (ms)</label>
            <input type="number" value={defaultDelay} onChange={(e) => setDefaultDelay(e.target.value)} min={10} step={10}
              className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-600/50 text-sm focus:outline-none focus:border-emerald-500/50" />
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Detected SSH Clients</label>
            <div className="space-y-1 max-h-32 overflow-y-auto">
              {clients.map((c) => (
                <div key={c.name} className="flex items-center justify-between px-2 py-1 rounded text-xs">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-slate-400">Custom SSH Clients</label>
              ...
              {customEntries.map((entry, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input value={entry.name} onChange={(e) => updateEntry(i, "name", e.target.value)}
                    placeholder="Name"
                    className="w-28 px-2 py-1.5 rounded bg-slate-800 border border-slate-600/50 text-xs focus:outline-none focus:border-emerald-500/50" />
                  <input value={entry.path} readOnly
                    placeholder="Click 📂 to select"
                    className="flex-1 px-2 py-1.5 rounded bg-slate-800 border border-slate-600/50 text-xs font-mono focus:outline-none truncate" />
                  <button onClick={() => browseFile(i)}
                    className="px-2 py-1.5 rounded bg-slate-700 hover:bg-slate-600 text-xs transition-colors shrink-0">📂</button>
                  <button onClick={() => removeEntry(i)}
                    className="px-2 py-1.5 rounded bg-slate-700 hover:bg-red-900/50 text-xs transition-colors shrink-0">✕</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-slate-700/50">
          ...
          <button onClick={save} className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-sm font-medium transition-colors">
            {saved ? "✓ Saved" : "Save"}
          </button>
        </div>
      </div>
