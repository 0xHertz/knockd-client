...
export default function ConnectionCard({ connection, onConnect, onEdit, onDelete, connecting }: Props) {
  ...
  try {
    ports = JSON.parse(connection.knockPorts);
  } catch { /* ignore */ }
  ...
  return (
    <div className="rounded-xl bg-slate-800/60 border border-slate-700/50 p-4 hover:border-slate-600/50 transition-colors">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">
              {isSsh && connection.username
                ? `${connection.username}@${hostDisplay}`
                : hostDisplay}
            </p>
          </div>
        </div>
        <span
          className={`text-[10px] px-2 py-0.5 rounded-full font-medium uppercase ${
            isSsh ? "bg-blue-900/50 text-blue-300" : "bg-purple-900/50 text-purple-300"
          }`}
        >
          {connection.connType}
        </span>
      ...
      {ports.length > 0 && (
        <div className="flex items-center gap-1.5 mb-3 flex-wrap">
          ...
          {ports.map((s, i) => (
            <span
              key={i}
              className="text-[10px] px-1.5 py-0.5 rounded bg-slate-700/50 text-slate-300 font-mono"
            >
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {connecting ? "Knocking..." : "🚀 Knock & Connect"}
        </button>
        <button
        ...
        <button
          ...
          className="px-3 py-2 rounded-lg bg-slate-700/50 hover:bg-red-900/50 text-slate-300 hover:text-red-400 text-sm transition-colors"
        >
          🗑️
        </button>
      </div>
